...
项目使用 Vision、Roadmap、Architecture、ADR、Design、Status、代码和测试分别保存长期目标、
能力顺序、跨阶段结构、决策取舍、具体契约、当前断点和实现证据。统一入口、单一事实来源、
三维状态模型、冲突处理与跨会话接力协议见 [Documentation Governance](governance.md)。

代码行为与已接受设计不一致时，必须显式判断这是实现缺陷还是设计已经改变，并相应更新
实现、测试、Design 或 ADR；不能以删除历史或静默改写文档制造一致。
