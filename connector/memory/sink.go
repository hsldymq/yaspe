...
// sinkState 保存同步 Sink 的接管结果, 失败计划和关闭等待状态.
// 除运行前配置的 beforeAccept 外, 所有字段由 mu 保护.
type sinkState[T any] struct {
	mu sync.Mutex
	// 是否成功绑定生命周期, 防止重复 Open 或未打开就接管.
	opened bool
	// 已停止新接管, 可以仍有已接受调用正在完成同步报告.
	closed bool
	// 接管前检查的生命周期, Close 时释放引用.
	lifecycle context.Context
	// 已完成接管的完整 Record 组, 按接管顺序追加, Close 后仍保留.
	groups [][]yaspe.Record[T]
	// 构造时复制的固定错误序列, nil 元素表示该次合法接管成功.
	failures []error
	// 下一条未消费计划的位置, 耗尽后不继续增长, 不是累计调用次数.
	failureIndex int
	// 非 nil 时拒绝所有合法接管, 与 failures 配置互斥.
	alwaysFail error
	// 已保存结果但尚未完成同步报告的 Accept 调用数, 不是 item 数.
	active int
	// 当前一批在途调用全部结束时关闭, active 从零变为非零时重新创建.
...
func (s *sinkState[T]) accept(ctx context.Context, items []yaspe.SinkItem[T]) ([]yaspe.SinkItemResult[T], error) {
	...
	for i, item := range items {
		group[i] = item.Record
		results[i] = yaspe.SinkItemResult[T]{
			Item:    item,
			Outcome: yaspe.SinkSucceeded,
		}
	}
